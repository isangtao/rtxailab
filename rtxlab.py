...
with st.sidebar:
    st.title("RTX AI Lab \n© 2024 Michael Carlos")
    mode = st.radio ("Modes", ('Chat', 'Internet Search', 'Agent', 'Image Generation', 'Vision', 'Voice Cloning', 'Source'))
    help = "help tbd"
    if mode == 'Chat':
    ...
    if mode == 'Internet Search':
        ...
        set_llm()
    if mode == 'Agent':
        help = "Super-smart assistants. Living F.A.Q.s. For the example ask the agent to: \n\nDescribe the property."
        set_llm()
...
def pictures():
    ...
    if prompt:
        negative_prompt = st.text_input("Negative Prompt e.g. extra fingers, extra arms, extra head, extra eyes, extra teeth, ugly, asymmetrical face")
